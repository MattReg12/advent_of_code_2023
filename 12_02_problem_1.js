...
let input = { 1: '3 blue, 2 green, 6 red; 17 green, 4 red, 8 blue; 2 red, 1 green, 10 blue; 1 blue, 5 green',
...
let sumGames = function(games) {
  let sum = 0;
  for (gameNum in games) {
    let redMax = max(games[gameNum].matchAll(/(\d+) red/g));
    let blueMax = max(games[gameNum].matchAll(/(\d+) blue/g));
    let greenMax = max(games[gameNum].matchAll(/(\d+) green/g));
    if (redMax <= RED_CUBES && blueMax <= BLUE_CUBES && greenMax <= GREEN_CUBES) {
      sum += +gameNum;
    }
  }
...
let max = function(regexMatches) {
  let counts = [...regexMatches].map(matchData => +matchData[1]);
  return Math.max(...counts);
}

console.log(sumGames(input));
