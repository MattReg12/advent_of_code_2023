...
const STRING_NUMBERS = { one: '1', two: '2', three: '3', four: '4', five: '5', six: '6',
                         seven: '7', eight: '8', nine: '9', eno: '1', owt: '2', eerht: '3',
                         ruof: '4', evif: '5', xis: '6', neves: '7', thgie: '8', enin: '9'};

let sum = function(input) {
  let nums = input.map(str => parseNums(str));
  return nums.reduce((total, strNum) => total += +strNum, 0)
}

let parseNums = function(string) {
  let regex = /one|two|three|four|five|six|seven|eight|nine|\d/;
  let reverseRegex = /eno|owt|eerht|ruof|evif|xis|neves|thgie|enin|\d/;
  let firstMatch = string.match(regex);
  let lastMatch = string.split('').reverse().join('').match(reverseRegex);
  let firstNum = STRING_NUMBERS[firstMatch[0]] || firstMatch;
  let lastNum = STRING_NUMBERS[lastMatch[0]] || lastMatch;
