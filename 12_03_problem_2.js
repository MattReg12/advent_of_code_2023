...
let surroundedNumCount = function(matchData) {
  ...
  let i = matchData['index'];
  let count = 0;
  for (point of BOX_INDICES) {
    if (/[0-9]/.test(String(string[i + point]))) {
      count += 1
    }
  }

  if (/\d{3}/.test(string.slice(i + 140, i + 143))) {
    count -= 1
  }
...
let surroundingNums = function(matchData) {
  let string = matchData['input'];
  let i = matchData['index'];
  let stringNums = CAPTURE_INDICES.map(indexArr => {
