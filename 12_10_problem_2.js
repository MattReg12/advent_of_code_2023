let input = 'F--F7F7|7FJ..JF-7F|77FL-L7.L.L7LFFJ-77|7.L777LF7L|FJJ-7-F|J7-7FF-7..FFF7JF--L77FL--77-7-77-|7FJ.LL.|7L7-|-FJJ.-J.F-77|F---|7.F--|--.F-7-7.F7\n' +
...
const COL_WIDTH_INCREMENTS = {right: [0, 1], left: [0, -1], up: [-1, 0], down: [1, 0]}
const NEXT = {right: 'left', left: 'right', up: 'down', down: 'up'}
const MOVES = {'-': {left: 'right', right: 'left'}, '|': {up: 'down', down: 'up'}, 'F': {right: 'down', down: 'right'},
               'L': {up: 'right', right: 'up'}, '7': {left: 'down', down: 'left'}, 'J': {up: 'left', left: 'up'}}
...
while (!(column === 42 && width === 25)) {
  let current = input[column][width];
  let next = MOVES[current][previous];
  input[column][width] = 'X';
  column += COL_WIDTH_INCREMENTS[next][0]
  width += COL_WIDTH_INCREMENTS[next][1]
  previous = NEXT[next];
}

input = input.map(x => x.join('')).join('\n')

input = input.replaceAll(/[7JFL\-|]/g, ' ')
